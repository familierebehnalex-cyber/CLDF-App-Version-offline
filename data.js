...
(() => {
  const DATA = window.CLDF_DATA || { dances: [], motionCatalog: [], specialRhythms: [], appVersion: '4.5.0', databaseVersion: 'unbekannt' };
  ...
  async function startLiveDanceRecognition() {
    ...
    if (!VM?.analyzeLiveVideo) return toast('Die MediaPipe-Liveanalyse ist nicht verfügbar.', 5000);
    state.videoBusy = true;
    ...
    try {
      ...
      if (error?.name !== 'AbortError') {
        console.error(error);
        setVideoRecognitionStatus('error', 'Live-Analyse fehlgeschlagen', error.message || 'Kamera oder MediaPipe konnten nicht gestartet werden.');
        toast(`Live-Analyse fehlgeschlagen: ${error.message || 'unbekannter Fehler'}`, 5000);
      } else {
  ...
  async function init() {
    ...
    runDiagnostics();
    handleInitialRoute();
    $('#versionText').textContent = `CLDF v4.5 · Offline · ${state.dances.length} lokale Tänze · ${GETINLINE_DANCES.length} Get-in-Line-Tänze · ${allFingerprintEntries().length} Audio-Referenzen · Liedzuordnung zuerst · BPM/Motion als Reserve`;
